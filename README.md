...
| トップ（デモの入口） | `https://<user>.github.io/<repo>/` | 2つのページへのボタン。お客さま用は管理者ページで最後に編集した内容で開く |
...
Place ID を入れなければ、デモの投稿先として **鈴与商事㈱**（`ChIJe_RRuvlJGmARhwimiSBHBOo`、依頼者指定）の「クチコミを投稿」画面が開く。
本物の投稿フォームなので、デモでは貼り付けたところで止めること。

## AI生成（Claude モード・BYOK）

- 管理者ページの「AI生成（Claude Haiku 4.5）」で、オンにして Anthropic API キーを入れる
- キーは**その端末の localStorage だけ**に置く。URL・リポジトリ・ビルド成果物・ログには出さない
- そのため Claude で作られるのは、キーを入れた端末でお客さま画面を開いたときだけ。ほかの端末（お客さまのスマホ）はテンプレートで作る
- モデル `claude-haiku-4-5`、`temperature: 1`、`max_tokens: 4000`。`strict: true` のツール `submit_review_drafts` で3案を受け取り、テンプレートと同じ検証に通す
- 失敗したとき・3案そろわないときはテンプレートで補う（画面は止めない）。下書きの右下に「Claude」「テンプレート」を表示する
...
| `src/core/claude.ts` | Claude モード（プロンプト・ツール・作り直し） |
| `src/core/engine.ts` | モードの切り替えとテンプレートへのフォールバック |
